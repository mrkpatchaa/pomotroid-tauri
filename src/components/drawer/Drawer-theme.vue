<template>
  <div class="Container">
    ...
    <div v-for="(t, i) in themer.themes" :key="i" class="Setting-wrapper" :style="
      ...
      <p class="Setting-title" :style="
                  `color: ${themer.getThemeValue(t, '--color-foreground')}`
                ">
        {{ themer.getThemeName(t) }}
      </p>
      <transition name="fade">
        <svg v-if="theme === themer.getThemeName(t)" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24"
          width="24">
          <path d="M0 0h24v24H0z" fill="none" />
          <path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z" fill="var(--color-accent)" />
        </svg>
      </transition>
    </div>
  </div>
